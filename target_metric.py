...
def pivot_target_column(df, target_name, player_name, id_name, agg_function):
    '''
    ...
    '''
    condition = list(set([target_name, player_name, id_name, 'season']) - set(list(df.columns)))
    if len(condition) > 0:
        sys.exit('Not all inputed column names are columns in df... Come on man!')

    # Filter out total team roles, only take 3 columns
    data_sub = df[df.team != 'TOT'][[player_name, id_name, 'season', target_name]]
    # Remove duplicates by aggregating down
    data_sub = data_sub.groupby([player_name, id_name,'season']).agg(agg_function).reset_index()
    ...
    player_seasons_full = pd.DataFrame(columns = [player_name, id_name, 'season'])
    for p in range(0, player_seasons.shape[0]):
        ...
        df_temp = pd.DataFrame(all_seasons, columns = ['season'])
        ...
        data_sub = (
            player_seasons_full
                .set_index([player_name, id_name, 'season'])
                .join(data_sub.set_index([player_name, id_name, 'season']), how = 'left')
                .reset_index()
        )
    ...
    for m in range(0, 9):
        if m <= 3:
            data_sub['season_minus_{}'.format(m+1)] = (
                data_sub.sort_values(by = ['season'])
