...
bbref_get_player_id_one_season = function(season) {
  ...
  player_df = player_df %>%
    tidyr::separate(player_name, c("first_name_punc", "last_name_punc"), sep = " ", remove = FALSE, extra = "merge")
  ...
  player_df = player_df %>%
    dplyr::mutate(first_name_lower = tolower(stringr::str_remove_all(first_name_punc, "[^A-Za-z]+")),
                  last_name_lower = tolower(stringr::str_remove_all(last_name_punc, "[^ -A-Za-z]")))
